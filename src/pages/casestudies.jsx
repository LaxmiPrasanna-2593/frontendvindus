...
const CaseStudies = () => {
  ...
  const featuredCaseStudies = caseStudies.filter((cs) => cs.title === 'Vaudit - SEO Tool' || cs.title === 'Qtipstore');
  ...
  <p>
    Since May 2023, Vindus Environment Pvt Ltd has been on a remarkable journey, helping clients achieve their goals through innovative solutions. 
    Under the visionary leadership of CEO Vinay Kalwala, our dedicated team has delivered outstanding results, transforming challenges into success stories across various industries.
  </p>
  <p>
    Our case studies showcase the impactful projects we've undertaken, highlighting the unique challenges faced by our clients and the tailored solutions we provided. 
    Explore how we have made a difference and continue to drive progress for businesses in an ever-evolving landscape.
  </p>
  <p>
          ...
          {featuredCaseStudies.map((caseStudy) => (
            <div key={caseStudy.id} className="case-study-card">
              ...
              <div className="card-content">
                ...
                <p>{caseStudy.scope}</p>
                <button className="read-more" onClick={() => setSelectedCaseStudy(caseStudy)}>Click for Details</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="all-case-studies">
        ...
        <div className="case-studies-grid">
          {filteredCaseStudies.map((caseStudy) => (
            <div key={caseStudy.id} className="case-study-card">
              <img src={caseStudy.visuals[0]} alt={caseStudy.title} className='gap' />
              <h3 className='gap'>{caseStudy.title}</h3>
              <p className='gap'>{caseStudy.client.name}</p>
              <p className='gap'>{caseStudy.scope}</p>
      ...
      {selectedCaseStudy && (
        <div className="case-study-modal">
          <div className="modal-content">
            <span className="close" onClick={() => setSelectedCaseStudy(null)}>
              &times;
            </span>
            <h2 className='gap'>{selectedCaseStudy.title}</h2>
            <h3 className='gap'>{selectedCaseStudy.client.name}</h3>
            <p className='gap'>{selectedCaseStudy.client.description}</p>
            <p className='gap'><strong>Timeline:</strong> {selectedCaseStudy.timeline}</p>
            <p className='gap'><strong>Scope:</strong> {selectedCaseStudy.scope}</p>
            <h4 className='gap'>Challenges:</h4>
            <ul>
              {selectedCaseStudy.challenges.map((challenge, index) => (
                <li key={index}>{challenge}</li>
              ))}
            </ul>
            <h4 className='gap'>Solutions:</h4>
            <ul>
              {selectedCaseStudy.solutions.map((solution, index) => (
                <li key={index}>{solution}</li>
              ))}
            </ul>
            <h4 className='gap'>Results:</h4>
            <p className='gap'>{selectedCaseStudy.results.description}</p>
            <p className='gap'><strong>Percentage Improvement:</strong> {selectedCaseStudy.results.percentageImprovement}</p>
            <h4 className='gap'>Sustainability Impact:</h4>
            <p className='gap'>{selectedCaseStudy.sustainabilityImpact}</p>
            <h4 className='gap'>Testimonial:</h4>
    ...
    <h3>Conclusion</h3>
    <p>In summary, our case studies showcase a diverse range of projects that demonstrate our expertise and commitment to delivering exceptional results for our clients. Each project reflects our dedication to understanding our clients’ unique needs and challenges, allowing us to tailor our solutions for maximum impact.</p>

    <p>Through our innovative approaches, we have consistently achieved significant outcomes, including:</p>
    <ul>
    ...
    <p>These case studies not only illustrate our capabilities but also serve as a testament to our collaborative approach with clients. We believe in fostering long-term partnerships that drive continuous improvement and success.</p>

    <p>If you are looking to achieve similar results for your business or have any questions about our services, we invite you to <a href="contact">get in touch with us</a>. Together, we can explore how we can help you overcome challenges and realize your business goals.</p>
</section>
